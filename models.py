...
def Kao_Onet_New(input_shape, output_size):
    input = Input(shape = input_shape)
    x = Conv2D(32, (3, 3), strides=1, padding='same', name='conv0')(input)
    # x = PReLU(name='prelu1')(x)
    x = PReLU(shared_axes=[1,2],name='prelu0')(x)
    x = Conv2D(32, (3, 3), strides=1, padding='valid', name='conv1')(x)
    # x = PReLU(name='prelu1')(x)
    x = PReLU(shared_axes=[1,2],name='prelu1')(x)
    x = MaxPool2D(pool_size=3, strides=2, padding='same')(x)
    x = Conv2D(64, (3, 3), strides=1, padding='valid', name='conv2')(x)
    ...
    # x = PReLU(name='prelu3')(x)
    x = MaxPool2D(pool_size=2)(x)
    x = Conv2D(128, (2, 2), strides=1, padding='same', name='conv4')(x)
    x = PReLU(shared_axes=[1,2],name='prelu4')(x)
    x = Conv2D(128, (2, 2), strides=1, padding='valid', name='conv5')(x)
    x = PReLU(shared_axes=[1,2],name='prelu5')(x)
    x = Flatten()(x)
    x = Dense(256, name='conv6') (x)
    x = PReLU(name='prelu6')(x)
    out = Dense(output_size)(x)
    model = Model(inputs=input, outputs=out)
    return model

def Kao_Onet_Deeper(input_shape, output_size):
    input = Input(shape = input_shape)
    x = Conv2D(32, (3, 3), strides=1, padding='valid', name='conv0')(input)
    # x = PReLU(name='prelu1')(x)
    x = PReLU(shared_axes=[1,2],name='prelu0')(x)
    x = Conv2D(32, (3, 3), strides=1, padding='valid', name='conv1')(input)
    # x = PReLU(name='prelu1')(x)
